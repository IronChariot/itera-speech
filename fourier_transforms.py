...
Who based theirs on Frank Zalkow, 2012-2013:
http://www.frank-zalkow.de/en/code-snippets/create-audio-spectrograms-with-python.html?ckattempt=2
"""
...
# For each wav file in one of the data folders, create a plot for it
data_contents = os.listdir(".")
sound_files = [filename for filename in data_contents
               if (filename.endswith(".wav") and
                   filename.startswith(sys.argv[1]))]

for sound_file in sound_files:
    # for augmentIdx in range(0, 20):
    #     alpha = np.random.uniform(0.9, 1.1)
    plotstft(sound_file, name=sound_file[:-4] + '.png')
    print sound_file
